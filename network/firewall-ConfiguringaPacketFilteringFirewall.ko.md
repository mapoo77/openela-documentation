...
목록의 모든 Zone을 구성할 수 있습니다. 특정 Zone의 설정을 변경하면 해당 Zone이 활성 Zone이 됩니다. 활성 Zone을 식별하려면 다음을 입력하십시오.:
...
기본적으로 모든 구성은 기본 Zone에서 구현됩니다. 또한 활성 Zone이 반드시 기본 Zone일 필요는 없습니다. 따라서 특정 Zone에 대한 설정을 정의하려면 명령에 Zone 이름을 지정해야 합니다. 그렇지 않으면 정의가 기본 Zone에 적용됩니다.
...
Zone을 지정하지 않으면 명령은 기본 Zone의 설정을 표시합니다. 따라서 `work` Zone의 설정을 나열하려면 다음 명령을 사용합니다.;
...
새로운 서비스에 대한 액세스를 열려면 `--add-service` _service_ 옵션을 사용하세요. 선택적으로 `--permanent` 옵션을 포함하여 재부팅 후에도 규칙이 지속되도록 합니다.
...
- `REJECT` 다른 규칙에서 허용한 트래픽을 제외한 모든 수신 트래픽을 차단합니다. 거부에 대한 정보가 원본 머신에 전달됩니다.

- `DROP`은 `REJECT`와 유사하지만 거부 알림이 원본 시스템으로 전송되지 않습니다.

### 소스를 기준으로 들어오는 트래픽 관리

트래픽 소스를 기반으로 Zone으로 들어오는 트래픽을 관리할 수 있습니다. 다음 두 가지 Zone 설정을 사용하면 패킷의 출처를 지정할 수 있습니다.:
...
'source-port' 설정을 지정하는 데 다음과 유사한 구문이 사용됩니다. 그러나 송신 포트 번호와 프로토콜 유형을 지정하여 소스 포트를 식별합니다. 예를 들어:
...
미리 정의된 Zone의 설정과 약간만 다른 Zone을 생성하는 경우 기존 구성 파일을 작업 디렉터리에 복사하는 것이 가장 쉬운 방법입니다. 다음 명령 중 하나를 사용할 수 있습니다.:
...
그런 다음 텍스트 편집기를 사용하여 새 구성 파일의 설정을 수정합니다. 다음 예는 `testzone`의 구성 파일에 포함될 수 있는 내용을 보여줍니다. `testzone`은 하나의 서비스\(SSH\)에 대한 트래픽과 TCP 및 UDP 프로토콜에 대한 하나의 포트 범위를 허용합니다.:
